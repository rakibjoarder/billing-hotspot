...
class Ppp_model extends CI_Model {
  ...
  public function create_ppp_user() {
    ...
    // while radius is disable
    if($radius_flag == 0  && $id_profile < 0) {
      $jsonRes = array('status' => 'failed', 'msg' => 'Failed !!!! Select profile!');
  ...
  // for last generated invoice date
    ...
    if($radius_flag == 1  && ($id_net_user_type == 1 || $id_net_user_type == 3 ) && (empty($net_user_ip_address) && $id_ip_pool == '-1')) {
      $jsonRes = array('status' => 'failed', 'msg' => 'You have to select IP pool or assign IP address !!');
      echo json_encode($jsonRes);
      return;
    }
...
public function edit_customer_now(){
  ...
  $id_ip_pool_prv = $this->get_id_ip_pool_from_net_user_profile($id_net_user);
  ...
  //
  if($radius_flag == 0  && ($id_net_user_type == 1 || $id_net_user_type == 3 ) && empty($id_profile)) {
    $jsonRes = array('status' => 'failed', 'msg' => 'You have to select Profile !!');
    echo json_encode($jsonRes);
    return;
  }
  //
  if($radius_flag == 1  && ($id_net_user_type == 1 || $id_net_user_type == 3 ) && (empty($net_user_ip_address) && empty($id_ip_pool))) {
    $jsonRes = array('status' => 'failed', 'msg' => 'You have to select IP pool or assign IP address !!');
    echo json_encode($jsonRes);
    return;
  }
  ...
  error_log('ID MK PPPOE ' . $id_mk_pppoe);
  if($sync_router_flag == true){

    if((($radius_flag == 1 || $radius_flag == 0) && $id_net_user_type == 2) || $radius_flag == 0){
      // mk login validation
      $mk_validation = $this->router_model->mikrotik_login_validation($router_ip,$router_login,$router_password);
    ...
    if($radius_flag == 1 && ($id_net_user_type == 1 || $id_net_user_type == 3) ){
      $db_edit_flag= true;
    }
    ...
    if(($id_net_user_type == 1 || $id_net_user_type == 3) && $radius_flag == 0){
      ...
      if($has_mikrotik_ppp){
        ...
        if(trim($res[0]) == "!done"){
          ...
          if($mk_delete_flag == "!done"){
            $this->db->update('net_user',array('id_mk_static' => '0'),array('id_net_user' => $id_net_user));
          }
      ...
      if($has_mikrotik_static){
        ...
        if(trim($res[0]) == "!done"){
          ...
          if($mk_delete_flag == "!done"){
            $this->db->update('net_user',array('id_mk_pppoe' => '0'),array('id_net_user' => $id_net_user));
          }
  ...
  if($db_edit_flag == true){
    ...
    if(!empty($id_ip_pool) && ($id_net_user_type ==1 || $id_net_user_type ==3 )  &&  ($id_ip_pool != $id_ip_pool_prv)){
      $updateFlag=1;
    }
   ...
   error_log('ooaodoooooooo' . $updateFlag);
    ...
    if($this->db->affected_rows()==0 && $updateFlag == 0){
      ...
      if($id_net_user_type==1 ||$id_net_user_type==3 ){
        ...
        if($id_profile > 0 || $id_ip_pool > 0){
          ...
          if(!empty($id_profile)){
            $sql_update['id_profile'] = $id_profile ;
          }
          if(!empty($id_ip_pool)){
            $sql_update['id_ip_pool'] = $id_ip_pool ;
          }
          ...
          if($this->db->affected_rows() > 0) {
            ...
            $this->db->update('net_user_profile', $sql_update ,array('id_net_user' => $id_net_user ));
          }else{
            //net_user profile entry
            $sql_update['id_net_user'] = $id_net_user ;
            $this->db->insert('net_user_profile', $sql_update);
          }
        }


        //if net_user_ip_address is null then update the radreply ip pool
        if(empty($net_user_ip_address) && $radius_flag == 1) {
          if($id_ip_pool > 0) {
            $this->db->select('*');
            $this->db->from('ip_pool');
            $this->db->where(array('id_ip_pool' => trim($id_ip_pool)));
        ...
        elseif(!empty($net_user_ip_address) && $radius_flag == 1){
          $this->db->select('*');
          $this->db->from('radreply');
        ...
        }else if ($radius_flag == 0){
          $this->radius_model->delete_ip_pool_radreply($id_router,$id_net_user);
          $this->radius_model->delete_ip_address_radreply($id_router,$id_net_user);
        }

        //for rad check
        if(strlen($net_user_password)!=0 && $radius_flag == 1){
          $this->db->select('*');
        ...
        }else if (strlen($net_user_password)==0 && $radius_flag == 1){
          $this->db->select('*');
          $this->db->from('radcheck');
          $this->db->where("id_net_user = '$id_net_user' and attribute = 'Cleartext-Password'");
          $this->db->get();
        ...
        if(strlen($net_user_mac)!=0 && $radius_flag == 1){
          $this->db->select('*');
          $this->db->from('radcheck');
          $this->db->where("id_net_user = '$id_net_user' and attribute = 'Calling-Station-Id'");
          $this->db->get();
          if($this->db->affected_rows()>0){
        ...
        }else if(strlen($net_user_mac)==0 && $radius_flag == 1){
...
public function get_net_users_by_zone_access($user_id){
  ...
  $this->db->where(array('zone_access.id_user' => $user_id, 'net_user.id_net_user >' => '0','net_user.id_isp' =>$this->session->userdata('id_isp')));
  ...
  function get_id_ip_pool_from_net_user_profile($id_net_user){
    $this->db->select('*');
    $this->db->from('net_user_profile');
    $this->db->where(array('id_net_user'=>$id_net_user));
    $query=$this->db->get();
