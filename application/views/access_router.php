<?php
	...
	if((permission("Use","Access_R_outer",$permission_string)===0) || permission("Use","View",$permission_string)=== -1){
		echo "<script>
				...
				<div class="card-header card-header-tabs" data-background-color="blue">
					<div class="nav-tabs-navigation">
						<div class="nav-tabs-wrapper">
							<span class="nav-tabs-title"><h1 class="card-title">Allow To <?=$user_info[0]['name']?></h1></span>
					...
					<div class="material-datatables">
						<table id="data_grid" class="table table-striped table-no-bordered table-hover" cellspacing="0" width="100%" style="width:100%">
							...
							<tbody>
								...
								?>
									<tr>
										<td><?php echo $router['name'] ?></td>
										<td><?php echo $router['ip_address'] ?></td>
										<td><?php echo $router['router_type_name'] ?></td>
										<td>
											<div class="togglebutton">
												<label>
													<input class="chkrouter" type="checkbox" value="" data-tag='<?php echo $router['id'] ?>' <?=($router['allowed']? 'checked': '')?>/>
												</label>
											</div></td>
									</tr>
									<?php
...
<script type="text/javascript">
	...
	$(document).ready(function() {
		...
		$(".chkrouter").on('click', function(){
			// alert('Item ID ' + $(this).attr('data-tag') + " Value is " + $(this).is(':checked'));

			$.ajax({
			 type:"POST",
			 dataType:"json",
			 data: {'router_id': $(this).attr('data-tag'), 'user_id': user_id},
			 url: '<?=base_url() ?>' + ($(this).is(':checked')? 'user/allow_router_access' : 'user/remove_router_access'),
