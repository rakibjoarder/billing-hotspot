                  ...
                  if(response['all_operations'][j].id_module == response['all_modules'][i].id_module){
                    ...
                    for(var k=0;k<response['indv_role_operations'].length;k++){
                      if( (response['indv_role_operations'][k].id_module == response['all_modules'][i].id_module ) && (response['indv_role_operations'][k].id_operation == response['all_operations'][j].id_operation )){
                          div_change=div_change+'<input type="checkbox" checked name="optionsCheckboxes" id="sub'+response['all_operations'][j].operation_name+'"id_operation="'+response['all_operations'][j].id_operation+'"id_module="'+response['all_modules'][i].id_module+'">';
                          flag=1;
                          break;
                      }
                    }
                    if(flag!=1){
                      div_change=div_change+'<input type="checkbox" name="optionsCheckboxes" id="sub'+response['all_operations'][j].operation_name+'"id_operation="'+response['all_operations'][j].id_operation+'"id_module="'+response['all_modules'][i].id_module+'">';
                    }
                    console.log("Ope:"+response['all_operations'][j].operation_name);


                    div_change=div_change+response['all_operations'][j].operation_name;
                    div_change=div_change+'</label>';
                    div_change=div_change+'</div>';
                    //checkbox
                  }
