<?php
	...
	if(permission("Rol","Edit",$permission_string)===0 || permission("Rol","View",$permission_string)=== -1){
		echo "<script>
...
<?php
	...
	foreach($role_info as $indiv):
    $id_role=$indiv['id_role'];
		$role_name=$indiv['role_name'];
		$role_desc=$indiv['role_desc'];
		$id_module=$indiv['id_module'];
	endforeach;
?>
			...
			<div class="card-content">
				...
				<form method="post"  id="form_admin" enctype="multipart/form-data">
					...
					<div class="form-group label-floating">
						...
						<input type="text" class="form-control" id="role_name" name="role_name"  value="<?php echo $role_name; ?>" required>
					</div>

					<div class="form-group label-floating">
						<label class="control-label">Description</label>
						<input type="text" class="form-control" id="role_desc" name="role_desc" value="<?php echo $role_desc; ?>">
					</div>
							...
							<select name="id_module" id='id_module' class="form-control" required>
								...
								<?php foreach ($modules as $module):
										if($id_module==$module['id_module'] ){
											?>
												 <option selected value="<?= $module['id_module']?>"><?= $module['module_name'] ?></option>
											<?php }else { ?>
		...
		$("#form_admin").validate({
			...
			submitHandler: function (form) {
				...
				$.ajax({
					...
					dataType:"json",
					data: reqData,
					url: "<?php echo base_url() ?>role/edit_role_now",
