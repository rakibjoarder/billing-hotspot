...
class Mapi_Ip_Firewall extends Mapi_Query {
     ...
     */
    public function unset_firewall_filter($id, $field){
        $input = array(
                    'command'   => '/ip/firewall/filter/unset',
                    'id'        => $id,
                    'value-name'=> $field
     ...
     * This method is used to add the firewall filter
     */
    public function move_firewall_filter($param){
        $input = array(
                    'command'   => '/ip/firewall/filter/move'
        );
        $out = array_merge($input, $param);
        return $this->query($out);
    }
